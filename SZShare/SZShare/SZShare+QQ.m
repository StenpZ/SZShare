...
+ (void)QQAuth:(NSString *)scope Success:(SZAuthSuccess)success Fail:(SZAuthFail)fail {
    if ([self beginAuth:schema Success:success Fail:fail]) {
        ...
        [self setGeneralPasteboard:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:schema][@"appid"]] Value:authData encoding:SZPboardEncodingKeyedArchiver];
 ...
 *  @return 需要打开的url
 */
+ (NSString *)genShareUrl:(SZShareMessage *)msg to:(int)shareTo {
    ...
    if (msg.link && !msg.multimediaType) {
        msg.multimediaType = SZMultimediaTypeNews;
    }
    if ([msg isEmpty:@[@"image",@"link" ] AndNotEmpty:@[@"title"]]) {
        ...
        [self setGeneralPasteboard:@"com.tencent.mqq.api.apiLargeData" Value:data encoding: SZPboardEncodingKeyedArchiver];
        ...
        if (msg.multimediaType == SZMultimediaTypeAudio) {
            msgType = @"audio";
        } else if(msg.multimediaType == SZMultimediaTypeVideo){
...
+ (BOOL)QQ_handleOpenURL {
    ...
    if ([url.scheme hasPrefix:@"QQ"]) {
        ...
        //登陆auth
        NSDictionary *ret = [self generalPasteboardData:[@"com.tencent.tencent" stringByAppendingString:[self keyFor:schema][@"appid"]] encoding:SZPboardEncodingKeyedArchiver];
