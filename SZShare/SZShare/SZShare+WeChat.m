...
#import "SZShare+WeChat.h"

@implementation SZShare (WeChat)
 ...
 *  @return 需要打开的url
 */
+ (NSString *)genWeixinShareUrl:(SZShareMessage *)msg to:(int)shareTo {
    ...
    if (msg.multimediaType == SZMultimediaTypeNews) {
    ...
    if (!msg.multimediaType) {
    ...
    } else if(msg.multimediaType == SZMultimediaTypeAudio) {
        //music
        dic[@"description"] = msg.desc?:msg.title;
        dic[@"mediaUrl"] = msg.link;
        ...
        dic[@"thumbData"] = msg.thumbnail? [self dataWithImage:msg.thumbnail]:[self dataWithImage:msg.image scale:CGSizeMake(100, 100)];;
        dic[@"title"] = msg.title;
    } else if(msg.multimediaType == SZMultimediaTypeApp) {
        //app
        dic[@"description"] = msg.desc?:msg.title;
        ...
        dic[@"thumbData"] = msg.thumbnail? [self dataWithImage:msg.thumbnail]:[self dataWithImage:msg.image scale:CGSizeMake(100, 100)];;
        dic[@"title"] = msg.title;
    } else if(msg.multimediaType == SZMultimediaTypeFile) {
        //file
        dic[@"description"] = msg.desc?:msg.title;
